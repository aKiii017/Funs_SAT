...
# 激活环境
conda activate base
cd /home/ubuntu/Fun_SAT/implementation/z3/build

# 在开始处理之前删除旧的results.txt文件，确保从干净的状态开始
rm -f results.txt

# 编译EasySAT程序
# make -C /home/ubuntu/FunSAT
make -j20
make_exit_status=$?

# 如果 make 命令失败，则输出错误信息并终止脚本执行
if [ $make_exit_status -ne 0 ]; then
  echo "Error: make command failed. Exiting script." >&2
  exit 1
fi
sudo make install
cd /home/ubuntu/Fun_SAT/implementation/z3/temp
# 在开始处理之前删除旧的results.txt文件，确保从干净的状态开始
rm -f results.txt
...
process_task() {
  ...
  # 运行EasySAT并捕获输出
  # output=$(timeout ${timeout_value}s z3 -wcnf -file "$file_name")
  output=$(timeout ${timeout_value}s python /home/ubuntu/Fun_SAT/implementation/z3/temp/test.py "$file_name")
  exit_status=$?
  end_time=$(date +%s.%N)
...
for file_name in "${!run_times[@]}"; do
  counter=$((counter + 1))
  printf "[TASK%03d]%s %s\n" "$counter" "$file_name"
  if [ ${sat_status_s[$file_name]} == "UNKNOWN" ]; then

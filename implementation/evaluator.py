...
def extract_cpp_function_body(cpp_code: str) -> str:
    """
    Extracts the body of the first function found in the given C++ code.

    Args:
    - cpp_code (str): The C++ code to process.

    Returns:
    - str: The extracted body of the function, including the surrounding braces.
    """
    start_index = cpp_code.find('{')
    if start_index == -1:
    ...
    while i < len(cpp_code) and open_braces > 0:
        if cpp_code[i] == '{':
            open_braces += 1
        elif cpp_code[i] == '}':
            open_braces -= 1
    ...
    if open_braces == 0:
        end_index = i
        return cpp_code[start_index:end_index]
    else:
        return "Function body not properly enclosed."
...
def _sample_to_program(
    ...
    body = extract_cpp_function_body(generated_code)
